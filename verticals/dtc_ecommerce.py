...
YOUR TASK: Find and analyze EXACTLY 3 DTC/eCommerce brands with $100M+ revenue.

CRITICAL RULES:
1. Do ONE web_search, then IMMEDIATELY pick 3 brands from those results
2. Do NOT do multiple searches - use the first result
3. For EACH brand: call analyze_site_geo, then extract_contacts
4. After 3 brands have extract_contacts called, call final_answer with JSON

WORKFLOW (MANDATORY):
- Turn 1: web_search
- Turn 2-7: analyze_site_geo + extract_contacts for brands 1-3 (2 turns each)
- Turn 8: final_answer
